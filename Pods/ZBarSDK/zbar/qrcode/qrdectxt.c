    ...
    if(!err){
      ...
      if(sa_size == 1)
          ...
          for(; syms; syms = syms->next) {
              ...
              next = (syms->next) ? syms->next->datalen : (unsigned int)sa_ntext;
      ...
      sa_sym->data = sa_text;
      sa_sym->data_alloc = (unsigned int)sa_ntext;
      sa_sym->datalen = (unsigned int)(sa_ntext - 1);
